...
export const RegistrationForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const committed = await addProfile(p);
    
    if (committed) {
      logActivity('REGISTRATION', `Personnel Entry Permanently Committed: ${p.fullName}`, p.id);
      setRegistryHash(Math.random().toString(16).substr(2, 8).toUpperCase());
      setSuccess(true);
      addToast("Registry Packet Saved Permanently.", "success");
    } else {
      addToast("Failed to link to central registry. Local cache active.", "error");
    }
    setLoading(false);
  };

  if (success) return (
    <div className="text-center p-12 bg-league-panel border-4 border-league-ok rounded-[3rem] animate-in zoom-in-95 max-w-2xl mx-auto relative overflow-hidden">
      <div className="absolute top-0 right-0 p-8 opacity-5 font-mono text-[10px] text-white rotate-12">IAL_SECURE_VAULT_v6</div>
      ...
      <h2 className="text-4xl font-black italic uppercase text-white mb-2">Vault Commit Successful</h2>
      <p className="text-league-ok font-mono text-[10px] uppercase tracking-widest mb-6">Handshake Verified: {registryHash}</p>
      <p className="text-league-muted uppercase tracking-widest text-[10px] mb-10 font-bold opacity-70 leading-relaxed">
        Your dossier is now permanently operational. Franchise GMs can access your metrics across the global node network.
      </p>
      <button onClick={() => setView('landing')} className="bg-league-accent text-white px-12 py-5 rounded-2xl font-black italic uppercase tracking-widest text-sm shadow-xl hover:brightness-110 active:scale-95 transition-all">Exit Induction Node</button>
    </div>
  );
    ...
    <form onSubmit={handleSubmit} id="draft-form" className="space-y-12">
      <div className="space-y-6">
        <div className="flex justify-between items-end mb-4">
           ...
           <span className="text-[8px] font-black uppercase text-league-muted bg-white/5 px-3 py-1 rounded-full border border-white/10 italic">Permanent Record Active</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Input label="Tactical Positions" value={formData.positions} onChange={(v: string) => setFormData({...formData, positions: v})} placeholder="QB, WR, LB" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {preferences.map((pref, i) => (
            <div key={i} className="space-y-2">
              ...
              <select className="w-full bg-league-bg border border-league-border p-3 rounded-xl text-white font-bold text-[10px] appearance-none focus:border-league-accent outline-none cursor-pointer" value={pref} onChange={e => {
                const newPrefs = [...preferences];
                const oldVal = newPrefs[i];
                const newVal = e.target.value as Franchise;
                const swapIdx = newPrefs.indexOf(newVal);
                newPrefs[i] = newVal;
                newPrefs[swapIdx] = oldVal;
      ...
      <div className="flex items-start gap-4 bg-black/40 p-8 rounded-3xl border border-league-border shadow-inner">
        ...
        <div className="space-y-2">
           <label className="text-[10px] font-bold text-league-muted uppercase leading-relaxed tracking-widest block cursor-pointer">
             I confirm I am 18+ and consent to being permanently archived in the IAL Registry for Draft 2026.
           </label>
        </div>
      </div>
      ...
      <button type="submit" disabled={!formData.consent || loading} className="w-full bg-league-accent text-white py-8 rounded-[2.5rem] font-black italic uppercase tracking-[0.5em] text-2xl shadow-[0_0_50px_rgba(228,29,36,0.3)] hover:brightness-110 transition-all disabled:opacity-30 flex items-center justify-center gap-4">
        {loading ? <div className="w-8 h-8 border-4 border-white border-t-transparent rounded-full animate-spin" /> : "COMMIT TO REGISTRY"}
      </button>
    </form>
  );
};
