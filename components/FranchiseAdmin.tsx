...
export const FranchiseAdmin: React.FC = () => {
  const { profiles, updateProfile, selectedFranchise, setSelectedFranchise } = useApp();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex gap-8 border-b border-league-border pb-6">
        {['Roster', 'Finance', 'Onboarding'].map((view: any) => (
          <button key={view} onClick={() => setActiveSubView(view)} className={`text-[11px] font-black uppercase tracking-[0.4em] transition-all relative ${activeSubView === view ? 'text-white' : 'text-league-muted hover:text-white'}`}>{view}{activeSubView === view && <div className="absolute -bottom-6 left-0 right-0 h-1 bg-league-accent shadow-[0_0_10px_#e41d24]" />}</button>
        ))}
      </div>
