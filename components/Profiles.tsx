...
const ProfileCard = React.memo(({ p, currentSystemRole, comparisonIds, toggleComparison, enrichDossier, maskPII }: any) => (
  <div key={p.id} className="bg-league-panel border border-league-border rounded-[2.5rem] p-8 group hover:border-league-accent transition-all flex flex-col shadow-2xl relative overflow-hidden h-full">
    ...
    <div className="flex items-center gap-6 mb-6">
      ...
      <div className="flex-1">
        <h4 className="text-xl font-black italic uppercase text-white tracking-tighter group-hover:text-league-accent transition-colors leading-none">{p.fullName}</h4>
    ...
    <div className="space-y-4 mb-6 flex-1">
       ...
       {p.aiIntel ? (
         <div className="bg-league-accent/5 p-4 rounded-2xl border border-league-accent/20 animate-in fade-in">
            ...
            <p className="text-[10px] font-bold text-white/80 italic leading-relaxed line-clamp-3">{p.aiIntel}</p>
         </div>
       ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredProfiles.map((p) => (
          <ProfileCard 
            ...
            enrichDossier={enrichDossier} 
            maskPII={maskPII} 
          />
        ))}
        {filteredProfiles.length === 0 && (
            <div className="col-span-full py-40 text-center bg-league-panel border border-league-border rounded-[3rem] opacity-20">
                <p className="text-xl font-black italic uppercase tracking-[0.4em]">No Registry Matches</p>
            </div>
        )}
      </div>
    </div>
